...
if (!LIST_PATH) {
  console.error('Usage: node scripts/enrich-skipped-deep-retry.mjs <files.txt>');
  process.exit(1);
}
...
const SEARCH_DELAY_MS = process.env.SEARCH_DELAY_MS ? Number(process.env.SEARCH_DELAY_MS) : 2000;
const SEARCH_RETRIES = process.env.SEARCH_RETRIES ? Number(process.env.SEARCH_RETRIES) : 8;
...
async function braveWebSearch(query) {
  ...
  for (let attempt = 1; attempt <= SEARCH_RETRIES; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        const backoff = Math.min(60000, 2500 * Math.pow(2, attempt));
        await sleep(backoff);
        continue;
      }
...
function scoreCandidate(url, name) {
  ...
  if (u.includes('tripadvisor') || u.includes('yelp') || u.includes('yellowpages')) s -= 10;
...
async function run() {
  ...
  const files = fs.readFileSync(path.resolve(LIST_PATH), 'utf8')
    .split(/\r?\n/)
    .map(s => s.trim())
    .filter(Boolean);
  ...
  async function worker(file) {
    ...
    try {
      ...
      const row = { file, status: 'error', error: String(e) };
      out.results.push(row);
      out.counts[row.status] = (out.counts[row.status] || 0) + 1;
    }
  }
  ...
  const outPath = path.join(ROOT, 'scripts', `deep-enrich-retry-report-${Date.now()}.json`);
